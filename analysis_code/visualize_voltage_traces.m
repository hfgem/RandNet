...
% 
% %Load data
% load(strcat(net_save_path,'/network_var.mat')) %want network_var(i).V_m for initialization i
...
%         hold on
    ...
    savefig(f,strcat(net_save_path,'/deflections_from_n',string(neur_ind(i)),'.fig'))
    saveas(f,strcat(net_save_path,'/deflections_from_n',string(neur_ind(i)),'.jpg'))
    close(f)
end
