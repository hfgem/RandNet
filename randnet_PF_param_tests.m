...
% num_nets = 4;
num_inits = 1;
test_n = 9; % Number of parameters to test (each)
...
variedParam(1).range = linspace( (220-50)*10^(-12), (220+50)*10^(-12), test_n); % set of values to test param2 at
...
if nargin == 2
    ...
    waitbar(p, H, ...
        ['Simulation ', num2str(COUNT), ' of ', num2str(TOTAL), ' complete', newline...
         'Runtime: ', datestr(datenum(0,0,0,0,0,toc),'DD:HH:MM'), ...
        ', Remaining time: ', datestr(datenum(0,0,0,0,0,toc/p-toc),'DD:HH:MM')]);
end
end
